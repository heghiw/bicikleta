...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _pickImage(bool isId) async {
    ...
    if (picked != null) {
      setState(() =>
          isId ? _idFile = File(picked.path) : _selfieFile = File(picked.path));
    }
  }
    ...
    try {
      if (widget.isLogin) {
        ...
        if (_idFile == null || _selfieFile == null) {
          setState(() {
            _error = 'Please upload your ID and selfie';
            _loading = false;
          });
          return;
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text(
                'Account created! Our team will verify your ID within 24 hours.'),
              ...
              children: [
                ...
                Center(
                  child: Container(
                    ...
                    child:
                        const Icon(Icons.pedal_bike, size: 34, color: psOrange),
                  ),
                ),
